...
def start(client, message):
    ...
    app.send_message(chat_id=message.from_user.id, text=f"Hello Netizen, I am **TikTok Mahier Bot**.\nAku bisa Download Video Tiktok tanpa Watermark.\n\n"
                          "__**Developer :**__ __@BagasMahier__\n"
...
def help(client, message):
    kb = [[InlineKeyboardButton('Channel 🛡', url=chnnl),InlineKeyboardButton('Repo 🔰', url="https://github.com/TerminalWarlord/TikTok-Downloader-Bot/")]]
    reply_markup = InlineKeyboardMarkup(kb)
    app.send_message(chat_id=message.from_user.id, text=f"Hello Netizen, I am **TikTok Mahier Bot**.\nAku bisa download semua vide tiktok dari link tiktok yang diberikan.\n\n"
                                            "__Kirim aku Link Tiktok__",
                     parse_mode='md',
                     reply_markup=reply_markup)
...
def tiktok_dl(client, message):
    a = app.send_message(chat_id=message.chat.id,
                         text='__Download sedang berjalan__',
                         parse_mode='md')
    ...
    with requests.get(r, timeout=(50, 10000), stream=True) as r:
        ...
        a.edit(f'__Download sedang berjalan!\n'
               f'Sedang Upload ke Telegram ⏳__')
        start = time.time()
        title = filename
